...
async def get_public_key(auth_host=Depends(get_auth_host)):
    pubkey_vo = await _auth_service.get_public_key(auth_host)
    return res_success(data=pubkey_vo)


# "meta": "{\"role\":\"teacher\",\"pass\":\"secret\"}"
@router.post("/signup", status_code=201)
async def signup(body: SignupVO = Body(...),
...
@router.post("/signup/confirm", 
             ...
             status_code=201)
async def confirm_signup(body: SignupConfirmVO = Body(...),
                          auth_host=Depends(get_auth_host_for_signup),
                          ):
...
@router.post("/login",
             ...
             status_code=201)
async def login(body: LoginVO = Depends(login_check_body),
                auth_host=Depends(get_auth_host),
                match_host=Depends(get_match_host),
                ):
    data = await _auth_service.login(auth_host, match_host, body)
    return post_success(data=data)


@router.post("/logout", status_code=201)
async def logout(role_id: int = Body(..., embed=True),
...
async def reset_password(
    ...
    await _auth_service.reset_passwrod(
